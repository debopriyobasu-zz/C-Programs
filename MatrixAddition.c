/*C program to add two mxn matrices
Version : 1.0
...
void addMatrices(int **x,int **y){
    int i,j;
    for(i=0;i<r;i++){
        for(j=0;j<c;j++){
//add the elements of the second matrix to the first matrix and store it in the first matrix
            *(*(x+i)+j) += *(*(y+i)+j);
        }
    }
